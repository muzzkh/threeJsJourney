...
const secitonMeshes = [mesh1, mesh2, mesh3]
...
const particleArr = new Float32Array(particlesCount * 3)

for (let i = 0; i < particlesCount; i++){
    const i3 = i*3
    particleArr[i3] = (Math.random()-0.5) * 10
    particleArr[i3+1] = .5 * objectsDistance - Math.random() * objectsDistance * secitonMeshes.length
    particleArr[i3+2] = (Math.random()-0.5) * 10
...
window.addEventListener('scroll', () => {
    ...
    if (newSection != currentSection){
        ...
        gsap.to(
            secitonMeshes[currentSection].rotation,
            {
                duration: 1.5,
                ease: 'power2.inOut',
                x: '+=6',
...
const tick = () =>
{
    ...
    //animate
    for (const mesh of secitonMeshes){
